...
public:
      ...
      const vector<Offer>& offers)
  {
    ...
    Resources taskResources = TASK_RESOURCES.get()
        .flattenSlack(Resource::RevocableInfo::ALLOCATION_SLACK);

    foreach (const Offer& offer, offers) {
      if (!launched &&
          Resources(offer.resources()).contains(taskResources)) {
        TaskInfo task;
        task.set_name(name);
...
int main(int argc, char** argv)
{
  ...
  framework.set_name("");
  framework.set_role("ads");
  framework.set_principal("p1");
  framework.set_checkpoint(flags.checkpoint);
  framework.add_capabilities()->set_type(
      FrameworkInfo::Capability::REVOCABLE_RESOURCES);

  Credential credential;
  credential.set_principal("p1");
  credential.set_secret("s1");
