...
class DockerDriver():
    ...
    def _start_container(self, container_id, instance_id, network_info=None):
        ...
        if not network_info:
            info = self._find_container_by_name(instance_id)
            self.docker.start(container=container_id,
                              network_mode='container:' + info['Name'],
    ...
    def spawn(self, context, instance_name, instance_id, command, image_name,
        ...
        bridge_container_id = self._create_bridge_container(instance_name,
                                                            instance_id)
        self._start_container(bridge_container_id, instance_id,
        ...
        working_instance_id = str(uuid.uuid4())
        image = self.docker.inspect_image(self._encode_utf8(image_name))
        if not (image and image['ContainerConfig']['Cmd']):
            args['command'] = ['sh']
        if command:
            args['command'] = command
        container_id = self._create_container(working_instance_id,
                                              image_name, args)
        if not container_id:
            raise exception.InstanceDeployFailure(
                _('Cannot create container'),
                instance_id=instance_id)
        self._start_container(container_id, instance_id)

    def _create_bridge_container(self, instance_name, instance_id):
        image_name = CONF.docker2.bridge_image
        args = {
            'hostname': instance_name,
            'network_disabled': True,
            'name':  self._encode_utf8(instance_id)
        }
