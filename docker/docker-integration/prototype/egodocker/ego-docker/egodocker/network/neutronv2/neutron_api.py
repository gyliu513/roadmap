...
from egodocker.i18n import _, _LE, _LW
from egodocker import context
from egodocker import exception
from egodocker.network import model as network_model
from egodocker.network import neutronv2
from egodocker import utils
from egodocker.network.neutronv2 import network
...
neutron_opts = [
    ...
    cfg.StrOpt('vif_driver',
            default='egodocker.network.neutronv2.vifs.DockerGenericVIFDriver'),
   ]
...
class API():
    def __init__(self):
        self.extensions = {}
        vif_class = importutils.import_class(CONF.neutron.vif_driver)
    ...
    def _nw_info_build_network(self, port, networks, subnets):
        ...
        network = network_model.Network(
            id=port['network_id'],
            bridge=bridge,
            injected=CONF.neutron.flat_injected,
            label=network_name,
            tenant_id=tenant_id
            )
    ...
    def plug_vifs(self, container_name, network_info):
        """Plug VIFs into networks."""
        for vif in network_info:
            self.vif_driver.plug(container_name, vif)
    ...
    def create_network_resource(self, context, container, tenant_id, host,
        ...
        nw_info = self.allocate(context, container_name, host,
                                tenant_id, **network_opts)
        if nw_info:
            self.plug_vifs(container_name, nw_info)
            self._attach_vifs(container_name, container, nw_info)
        return nw_info


    def delete_network_resource(self, context, container, tenant_id, **network_opts):
        container_id = container['id']
        container_name = container['name']
        nw_info = self.get_network_info(context, container_name,
                                       tenant_id, **network_opts)
        network.teardown_network(container_id)
        self.unplug_vifs(container_name, nw_info)
        self.deallocate(context, container_name)
